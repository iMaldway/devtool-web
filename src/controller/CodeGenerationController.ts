...
export default class DatabaseController {
  ...
  getTableRow = async (key: any) => {
    if (key || key === 0) {
      const activeItems = this.selectDataBaseList.value.filter((item: any) => item.id && item.id === key)
      ...
      if (this.tableInfo.value.database != itme.name) {
        ...
        const res = await DevtoolGeneratorDatabase.getTables(this.tableInfo.value)
  ...
  getList = async () => {
    ...
    const res = await DevtoolGeneratorDatabase.getTableInfo(this.tableInfo.value)
   ...
   */
  generateCode = async () => {
    if (this.tableInfo.value.database == undefined || this.tableInfo.value.tableName == undefined) {
      ElMessage({
        type: 'warning',
        message: '请选择好数据库以及表'
      })
  ...
  buildByAnalysis = async () => {
    if (this.pageForm.value.templateName == undefined || this.pageForm.value.analysisId == undefined) {
      ElMessage({
        type: 'warning',
